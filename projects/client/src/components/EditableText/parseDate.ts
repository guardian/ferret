...
const dayMonthYearDateRegex = new RegExp(
	'^(\\d+)' +
...
export function parseDate(text: string, mode: 'from_start' | 'from_end') {
...
function validateMonth(month: string) {
	...
	return (
		!!num ||
		monthNames.some(monthTexts => monthTexts.includes(month.toLowerCase()))
	);
}
